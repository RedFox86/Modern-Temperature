...
public class PlayerTemperature {
  ...
  public static float calculateTemperatureGoal(ServerPlayer player) {
    ...
    // Fluids
    String insideBlock = getBlockNameRelativeToPlayer(level, player, Vec3i.ZERO);
    goalTemperature += returnMapValueIfKeyExists(FLUID_TEMPERATURE_VALUES, insideBlock);

    if (player.getItemBySlot(EquipmentSlot.FEET).isEmpty()) {
      // Snow
      goalTemperature += returnMapValueIfKeyExists(WALKING_ON_TOP_MINI_BLOCKS_VALUES, insideBlock);

      // Ice, packed ice, blue ice
      goalTemperature +=
          returnMapValueIfKeyExists(
              WALKING_ON_TOP_VALUES,
